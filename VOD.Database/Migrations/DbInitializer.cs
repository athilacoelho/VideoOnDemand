...
namespace VOD.Database.Migrations
{
    public class DbInitializer
    {
        ...
        {
            var description = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sin est etiam corpus, " +
                "ista explanatio naturae nempe hoc effecerit, ut ea, quae ante explanationem tenebamus, relinquamus. " +
                "Itaque non discedit ab eorum curatione, quibus praeposita vitam omnem debet gubernare, ut mirari satis " +
                "istorum inconstantiam non possim. Pauca mutat vel plura sane; Quae fere omnia appellantur uno ingenii nomine, " +
                "easque virtutes qui habent, ingeniosi vocantur. Et hunc idem dico, inquieta sed ad virtutes et ad vitia " +
                "nihil interesse. Duo Reges: constructio interrete. ";
            var email = "a@b.c";
            ...
            if (!userId.Equals(string.Empty))
            {
                ...
                {
                    if (!courseId1.Equals(int.MinValue)) context.UserCourses.Add(new UserCourse { UserId = userId, CourseId = courseId1 });

                    if (!courseId2.Equals(int.MinValue)) context.UserCourses.Add(new UserCourse { UserId = userId, CourseId = courseId2 });

                    if (!courseId3.Equals(int.MinValue)) context.UserCourses.Add(new UserCourse { UserId = userId, CourseId = courseId3 });
                ...
                if (!context.Modules.Any())
                {
                    var modules = new List<Module>
                    {
                        ...
                        new Module { Course = context.Find<Course>(courseId2), Title = "Module 2"},
                        new Module { Course = context.Find<Course>(courseId3), Title = "Module 3"}
                    };
                ...
                if (!context.Videos.Any())
                {
                    ...
                    if (!context.Downloads.Any())
                    {
                        var downloads = new List<Download>
                        {
                            new Download 
                            {   
                                ModuleId = moduleId1, CourseId = courseId1, Title = "ADO.NET 1(PDF)", Url = "https://some-url"
                            },
                            new Download
                            {
                                ModuleId = moduleId1, CourseId = courseId1, Title = "ADO.NET 2(PDF)", Url = "https://some-url"
                            },
                            new Download
                            {
                                ModuleId = moduleId3, CourseId = courseId2, Title = "ADO.NET 1(PDF)", Url = "https://some-url"
                            },
                        };
